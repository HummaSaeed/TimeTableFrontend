...
const EditClass = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(error.response?.data?.error || error.response?.data?.message || 'Failed to update class');
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchClassDetails = async () => {
        try {
            ...
            setFormData({
                ...
                total_strength: classData.total_strength || '',
                class_teacher: classData.class_teacher || '',
                selected_subjects: classData.subjects ? classData.subjects.map(s => s.id) : [],
    ...
    const resetForm = () => {
        fetchClassDetails(); // Reset to initial fetched data
        setSubjectTemplate('');
    };
    ...
    if (initialLoading) {
        return (
            <div style={{
                ...
                <Spinner animation="border" style={{ color: '#1A6E48' }} />
            </div>
        );
    }
                    ...
                    <Card style={{
                        ...
                        <Card.Body style={{ padding: '32px' }}>
                            ...
                            {success && (
                                ...
                                >
                                    Class updated successfully! Redirecting to classes list...
                                </Alert>
                            )}
                                ...
                                <Row>
                                    ...
                                    <Col md={12} className="mb-4">
                                        ...
                                        <Form.Group>
                                            ...
                                            <div className="mb-3">
                                                <div className={`d-flex align-items-center gap-2 p-2 rounded ${formData.selected_subjects.length >= 5 && formData.selected_subjects.length <= 8
                                                        ? 'bg-success bg-opacity-10 text-success'
                                                        : formData.selected_subjects.length < 5
                                                            ? 'bg-warning bg-opacity-10 text-warning'
                                                            : 'bg-danger bg-opacity-10 text-danger'
                                                    }`}>
                                                    <i className={`fas ${formData.selected_subjects.length >= 5 && formData.selected_subjects.length <= 8
                                                            ? 'fa-check-circle'
                                                            : formData.selected_subjects.length < 5
                                                        ...
                                                        {availableSubjects.map((subject) => {
                                                            ...
                                                            return (
                                                                <div key={subject.id} className="col-md-4 col-lg-3 col-xl-2">
                                                                    <div
                                                                        className={`subject-card p-2 rounded border cursor-pointer transition-all ${isSelected
                                                                    ...
                                                                    >
                                                                        <div className="text-center w-100">
                                                                            <div className="mb-1">
                                                                                <i className={`fas fa-book ${isSelected ? 'text-white' : isDisabled ? 'text-muted' : 'text-primary'
                                                                                    }`} style={{ fontSize: '1.2rem' }}></i>
                                                                            </div>
                                                                            <h6 className={`mb-1 fw-bold small ${isSelected ? 'text-white' : isDisabled ? 'text-muted' : 'text-dark'
                                                                                }`} style={{ fontSize: '0.8rem', lineHeight: '1.1' }}>
                                                                                {subject.name}
                                                                            </h6>
                                                                            <Badge
                                                                                bg={isSelected ? 'light' : isDisabled ? 'secondary' : 'outline-secondary'}
                                                                                className={`small ${isSelected ? 'text-dark' : isDisabled ? 'text-white' : 'text-secondary'
                                                                                    }`}
